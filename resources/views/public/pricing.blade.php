...
<section class="relative overflow-hidden pt-28 pb-16 lg:pt-36">
    ...
    <div class="land-section text-center">
        ...
        {{-- Billing toggle --}}
        <div x-data="{ annual: false }" class="mt-10">
            <div class="inline-flex items-center gap-3 rounded-2xl border border-white/10 bg-white/[0.03] p-1.5">
                <button
                    @click="annual = false"
                    :class="!annual ? 'bg-brand-500 text-white shadow-glow-orange-sm' : 'text-slate-400 hover:text-slate-200'"
                ...
                @if ($plans->isNotEmpty())
                    @foreach ($plans as $i => $plan)
                        ...
                        <div class="price-card {{ $popular ? 'price-card-popular relative' : '' }}">
                            ...
                            <div class="mt-5">
                                <div class="flex items-baseline gap-1" x-show="!annual">
                                ...
                                <div class="flex items-baseline gap-1" x-show="annual" style="display:none">
                                ...
                                <p class="mt-1 text-xs text-slate-600" x-show="annual" style="display:none">Billed annually ({{ $plan->currency }} {{ number_format((float)$plan->price_amount * 0.8 * 12, 0) }}/yr)</p>
                            </div>
            ...
            @foreach ([
                ...
                <div class="rounded-2xl border border-white/10 bg-white/[0.02] overflow-hidden">
                    <button
                        ...
                        class="flex w-full items-center justify-between px-6 py-4 text-left text-sm font-semibold text-white"
                    >
                        {{ $q }}
                        <svg :class="open === {{ $idx }} ? 'rotate-180' : ''" class="h-4 w-4 shrink-0 text-slate-400 transition-transform duration-200" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M19.5 8.25l-7.5 7.5-7.5-7.5" /></svg>
                    </button>
                    <div
                        x-show="open === {{ $idx }}"
                        ...
                        class="px-6 pb-5 text-sm text-slate-400 leading-relaxed"
                    >
                        {{ $a }}
                    </div>
                </div>
